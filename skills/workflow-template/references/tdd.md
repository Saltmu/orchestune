...
4. **Bloat Warnings**: If file size or complexity warnings are detected, **do not pause for user approval**; autonomously perform modular refactoring to resolve new or worsened warnings (if unresolved after 3 attempts, pause work and escalate). After refactoring, re-run verification procedures (steps 1–3) to ensure no regressions were introduced. Distinguish between pre-existing baseline warnings and newly introduced/worsened warnings for context.
