...
## 軽微変更（Typo / Docs）の短経路
コードロジックの変更を伴わないドキュメント修正や typo 修正では、**ステップ3〜8（TDD）をスキップ**して構いません。ただし、機密情報漏洩防止（gitleaks）や品質保証のため、**ステップ9のローカルCI（`./scripts/local-ci.sh` / `.\\scripts\\local-ci.ps1`）は必ず実行**した上でステップ10（PR作成）へ進んでください。
