...
- **AI・外部CIレビュー待機時のタスク多重起動防止**:
  `@claude review` や GitHub Actions の完了を待機する際は、`schedule` によるタイマーループやポーリングコマンドの連打（プロセスの多重起動・割り込み多重化）を厳禁とします。必ず `poetry run python scripts/wait_for_review.py --pr <PR番号>` または単一のブロッキングスクリプトを1回だけ実行し、追加のコマンドを発行せずに完了通知（Reactive Wakeup）を1度だけ受け取る手順を徹底してください。
