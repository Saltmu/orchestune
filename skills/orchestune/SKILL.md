...
1. Delegate consistency validation of the `decomposition_plan.md` to the `orchestune-dag` CLI (this is the "ask orchestune-dag to decompose/validate" step — `orchestune` never re-implements DAG validation itself):
   ...
   * If the output includes a `Warnings:` section, `orchestune-dag` has detected two or more subtasks that both actually *write* to the same shared extension point (either tagged with the same `shared_contract` and each writing to it per the footprint/`writes_shared_contract` check described in Stage 1, or — for untagged subtasks — whose declared `footprint` entries fall into the same shared-extension-point category *and* directory: registry, CLI wiring, public API index, dependency manifest), and that are not ordered relative to each other in the DAG (neither is reachable from the other via `depends_on`/inferred edges — having a common ancestor task is not enough, since siblings of a shared ancestor can still run in parallel). Tagged subtasks that only depend on the contract without writing to it (pure consumers) are never part of this warning. This is not a blocking error, but it should normally be resolved by revising `decomposition_plan.md` (add a `depends_on` edge directly between the affected writer subtasks, turn a writer into a pure consumer if it doesn't actually need to touch the shared file, or confirm the paths genuinely refer to unrelated files) before moving to Stage 3.
