...
1. Delegate consistency validation of the `decomposition_plan.md` to the `orchestune-dag` CLI (this is the "ask orchestune-dag to decompose/validate" step — `orchestune` never re-implements DAG validation itself):
   ...
   * A single `Warnings:` output can combine more than one of the following warning types at once — check each entry against its own wording below rather than assuming they're all the same kind:
     ...
     - **Existence-verification warning** (#393/#400): an entry reading `<subtask-id>: footprintに実在しないパスがあります` (for `footprint`) or `<subtask-id>: symbolsが実コードベースに見つかりません` (for `symbols`). This is a distinct check from the shared-contract warning above — `orchestune-dag` checked whether the declared path/symbol is actually present in today's codebase, not whether it collides with another subtask. Do not treat it as a shared-contract warning (it is not about ordering two writers), and do not silently skip past it either — triage each occurrence:
       * **The subtask is about to create this path/symbol for the first time**: no action needed. A `footprint`/`symbols` entry for a not-yet-existing file or symbol is expected to be reported as "missing" — that's exactly what a new-file/new-symbol subtask looks like.
