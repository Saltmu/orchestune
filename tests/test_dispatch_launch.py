...
class TestApplyTaskLaunches:
    ...
    def test_active_worktree_started_at_uses_per_task_launch_time_not_cycle_now(
        self, tmp_path
    ):
        ...
        誤ってstale扱いする境界ケースを避けるため、タスクごとに実起動直前の
        `time.time()`を記録する。"""
        from unittest.mock import MagicMock, patch
        ...
        run_state = RunState(active_worktrees={})
        cycle_now = 1_000.0
        per_task_time = 1_050.0

        with (
            patch("orchestune.dispatch_worktree._branch_exists", return_value=False),
            patch("orchestune.dispatch_worktree.subprocess.run") as mock_run,
            patch("orchestune.dispatch_targets.subprocess.Popen") as mock_popen,
            patch("orchestune.dispatch_launch.time.time", return_value=per_task_time),
        ):
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
            mock_popen.return_value.pid = 1234
        ...
        assert active.started_at == per_task_time
