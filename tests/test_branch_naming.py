...
class TestBuildTaskBranchName:
    def test_default_prefix_matches_historical_claude_convention(self) -> None:
        """既定値は`claude`のまま。既にpush済みのin-flightタスクのブランチと、
        Claude Codeクラウドルーチンの`claude/`限定push制限の双方に合わせる。"""
...
class TestBranchMatchesTask:
    ...
    def test_rejects_unparsable_branch(self) -> None:
        """`Closes #N`だけを持つ無関係なブランチ名は形状不一致で弾く
        （`_is_orchestune_issue_branch`の安全弁が緩まないことの根拠）。"""
        assert not branch_matches_task("main", 42)
        assert not branch_matches_task("some-unrelated-branch", 42)
