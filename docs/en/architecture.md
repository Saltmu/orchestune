...
**Observability and rollback**: every candidate — selected or not — contributes its score breakdown, bottom level, release counts, cost estimates and skip reason (`conflict` / `quota-exhausted` / `token-budget` / `launch-failed`) to the cycle report, the `--json` output and `events.jsonl`. Scheduling selection and actual launch are distinct: a task whose launch slot could not be reserved, or whose `create_worktree_and_launch` failed, is downgraded to `launch-failed` by `reconcile_decisions_with_launches`, so the report never disagrees with `CycleReport.selected`. `--scheduling-mode legacy` restores the pre-#660 scoring.
