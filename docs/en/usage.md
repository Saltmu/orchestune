...
| `--dispatch-target {local,cloud-routine,claude-cli,agy-cli}` | auto-selected (non-CI: `claude-cli` / GitHub Actions: `cloud-routine`) | Target environment to launch agents. When unspecified, it is auto-selected from the runtime environment (the `GITHUB_ACTIONS` variable). Dispatch to a local `claude`/`agy` CLI with a built-in preset command that always passes a permission-bypass flag (scoped by the subtask's own worktree), or dispatch to Claude Code Cloud Routine. Only explicitly passing `local` gives the backward-compatible no-op dummy (for tests/dry-runs). |
