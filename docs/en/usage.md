...
1. **When a PR is opened**: once the dispatcher sees an open PR targeting `parent/issue-{N}` whose head lives in the upstream repository (PRs from forks, and any PR whose head origin cannot be confirmed, are skipped so that a third party cannot post an authoritative-looking notice on someone else's issue), it posts a "PR #XXX has been opened" notice on the corresponding child issue. The target issue is resolved both from the PR's `Closes #N` references and from the head branch name (`claude/issue-{N}-{subtask_id}`), so PRs opened by the agent itself are announced through the same path.
