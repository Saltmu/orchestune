...
> [!NOTE]
> `orchestune provision` によるIssue番号の書き戻し（`parent_issue_number`・各サブタスクの `issue_number`）は、上記フォーマット例のような**標準的なブロックスタイルYAML**（各サブタスクを `- key: value` の複数行で記述し、キーは非クォートの識別子）を前提としています。フロースタイル（`- {id: task-a, ...}`）の単一行マッピングにも対応していますが、複数行にまたがるフローマッピング、クォート済みキー（`"id": task-a`）、サブタスク項目の途中に挟まれた行頭コメントなどの非標準的な記法はサポート対象外です。承認済みplanは上記の標準的な記法で記述してください。
