...
**LLMには状態を変更する権限がありません。** 例えば統合コーディネーター（セクション3）が起動する意味的レビューのセッションは、統合PRへ所見をコメントするだけで完結し、ラベル付与・マージ・Issueのクローズは一切行いません。Python側がラベルをポーリングし、決定論的に次の状態へ遷移させます。
...
| 宣言の誤り（footprint外への変更） | 逸脱検知（セクション3） | DAG再計算（デッドバンドと回数上限つき） |
| インフラの失敗（ローカル状態の消失） | — | GitHubを真実とする再構築（セクション2） |
| エージェントの自己申告 | 記憶を持たない独立セッションによる再検証 | ラベル経由でPython側が決定論的にクローズ |

そして、**すべてのループに上限があり、終端があります**。再計算の回数、ウィンドウあたりの起動数とトークン消費、タスクのタイムアウトはいずれも有界で、微小な逸脱はデッドバンドで無視することでライブロックを防ぎます。それでも自動的に収束できない場合、対象Issueは`status:blocked-human-review`へ遷移して停止します。
...
リファクタ（ファイル分割・関数の移動・リネーム）を経た分解計画は、既に存在しないコードスナップショットを指していることがあります。`orchestune/symbol_verification.py`はIssue生成時に、宣言された`footprint`のパスと`symbols`が実際のコードベースに見つかるかをASTで検証します。

シンボルの収集はスコープを認識して行います。`ast.walk`は木全体をフラットに走査してスコープ情報を失うため、クラスメソッドの裸名や関数内のローカル変数をモジュールレベルの定義と取り違えます。一方で`if`/`try`/`with`/ループ/`match`は、囲むブロックと同じスコープへ定義を束縛するため、その内側は平坦化して取り込む必要があります（条件付きの`import`やメソッド定義が典型例）。関数・クラス定義はそれ自体が新しいスコープを作るため、そこへは再帰しません。
