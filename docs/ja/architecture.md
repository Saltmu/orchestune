...
Orchestuneが**目指す**のは「常に自動で解決すること」ではなく、**収束するか、人間が対処可能な状態で停止するかのいずれかであること**です。ただしこれは設計目標であって、現時点で全経路が満たしているわけではありません——上記「既知の欠落」の2経路に加え、`--task-timeout-seconds`を既定の0のままハングした通常タスクも、まだこの目標の外にあります。
