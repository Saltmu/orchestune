...
2. **`git` / `gh` の実行はL1に閉じ込める**:
   ...
   対象はVCS・GitHubクライアントの表面のみです。それ以外の外部プロセス起動は意図的に対象外としており、ガードもしていません。具体的には、`dispatch.targets` はエージェントのCLIを起動し、`dispatch.rebase` はCIスクリプトを実行します。Poetryによる依存関係と仮想環境の操作は、L1アダプタの `infra.python_env` にカプセル化しています。
