...
**共有状態の自動遷移はPython側が行います。** 例えば統合コーディネーター（セクション3）が起動する意味的レビューのセッションは、統合PRへ所見をコメントするだけで完結し、ラベル付与・マージ・Issueのクローズは一切行いません。

ただしこのレビューは**fire-and-forgetで、Python側が結果を追跡することもありません**。所見は統合PRへのコメントとして残るだけで、自動マージを止めたり変えたりはしません。

所見が検収者の目に入るかは統合モードで変わります。`--parent-issue`を指定しない**フラットモード**では、その統合PR自体が人間のマージする検収PRなので、所見は同じPR上にあります（`AutoMergeChildIntegrationStep`は`parent_issue_number`が`None`のとき即座に戻り、自動マージしません）。一方**親ブランチによる二層モデル**（`--parent-issue`指定時）では、所見は子の統合PRに付き、その直後に自動マージが走ります。検収PR（親ブランチ→`main`）は別物で、**転記もリンクもされません**——非同期の所見が子PRのクローズ後に届くこともあるため、読むには子PRを個別に辿る必要があります。

状態機械へ結果が戻る**LLMレビュー**は、後述の`status:not-needed`検証レビュー（ラベル経由）だけです。人間のレビューは別で、子PRの`CHANGES_REQUESTED`は`_rule_changes_requested`が拾い、依存する実行中の作業を止めて対象Issueを`status:blocked-human-review`へ遷移させます。
...
**検証がスキップされる条件が2つあります**。footprintに実在する`.py`ファイルが1つも無い場合と、footprint中のいずれかの`.py`がパース不能（構文エラー等）だった場合で、どちらも空の結果を返し注記は付きません——判定材料が欠けた状態で「存在しない」と断定するとfalse positiveになるためです。ただし**リファクタでファイルが分割・改名された直後は、まさにfootprintのパスが実在しない状態**になり得ます。この検証が最も効いてほしい場面ほど、静かに保留されやすいということです。
