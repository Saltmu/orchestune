...
ただしこのレビューは**fire-and-forgetで、Python側が結果を追跡することもありません**。検出されたバグは統合PRのコメントとして残るだけで、自動マージを止めたり変えたりはしません——拾うのは検収マージを行う人間です。状態機械へ結果が戻るのは、後述の`status:not-needed`検証レビュー（ラベル経由）だけです。
