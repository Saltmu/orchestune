...
def _provision_and_link_one_subtask(
...
def _prepare_provisioning_context(
    plan_path: str | Path,
    template_path: str | Path,
    repo_root: str | Path | None,
    parent_issue: int | None,
) -> tuple[Path, list[SubTask], PlanMetadata, int | None, DagResult, str]:
    ...
    validated_parent = (
        validate_issue_number(parent_issue) if parent_issue is not None else None
    )
    ...
    return resolved_repo_root, subtasks, metadata, validated_parent, dag, template


def _apply_provisioning(
    forge: IssueForge | None,
    metadata: PlanMetadata,
    plan_path: str | Path,
    validated_parent: int | None,
    dag: DagResult,
    template: str,
    resolved_repo_root: Path,
) -> ProvisionResult:
    ...
    (
        created,
        reused,
        degraded_subtask_ids,
        loop_plan_synced,
    ) = _provision_subtasks_loop(
        resolved_forge,
        dag,
        template,
        resolved_repo_root,
    ...
    return ProvisionResult(
        ...
        plan_synced=parent_plan_synced and loop_plan_synced,
    )
...
def provision_issues(
    ...
    (
        ...
        dag,
        template,
    ) = _prepare_provisioning_context(plan_path, template_path, repo_root, parent_issue)
