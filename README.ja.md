...
## 設計思想: 「開発を速くする」ためではなく「クオーターを効率良く使う」ため

Orchestuneは、チーム規模のプラットフォームではなく、**限られたAI利用クオーター（サブスクリプションのセッション/週次の使用枠など）の中で作業する個人開発者向けのオーケストレーター**です。その目的は、個々のタスクを実時間で速く終わらせることでは**ありません**。小さなタスクであれば、分解とディスパッチを経由するよりも、エージェントに直接依頼したほうが速く終わります。

Orchestuneが最適化するのは、**消費したクオーターのうちどれだけが「マージ可能な完成物」に変わるか**です。大きなタスクを独立したサブタスクへ分解すれば、複数のエージェントを並列に——無人で、バックグラウンドで、あるいは夜間に——走らせられます。1本の長い直列セッションでプロンプトの合間にクオーターを遊ばせるのではなく、同時実行にクオーターを注ぎ込めるということです。DAG検証、自己修復によるステートリカバリ、人間の判断を2点に絞る承認モデル（[アーキテクチャと設計思想](docs/ja/architecture.md)参照）は、いずれもこの並列実行を**無人で放置できるだけ安全にする**ために存在します。それが実現できて初めて、クオーター効率の向上は実際の利得になります。
