...
そしてパイプラインを実際に進めるのは決定論的なPythonです。LLM呼び出しはクオーターを消費する希少な操作なので、判断が代替不能な箇所にのみ使い、共有状態の自動遷移はPython側が担います（[0.1 決定論](docs/ja/architecture.md#01-決定論-llmは判断共有状態の自動遷移はpython)）。
